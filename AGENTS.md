...
- Stripe renders the checkout UI as its embedded form inside our own page at /prodotto/[slug]/checkout: `ui_mode: "form"` on the session, `stripe.initCheckoutFormSdk` + `createForm` on the client (src/components/checkout/EmbeddedCheckoutMount.tsx). There is no redirect to a Stripe-hosted page.
- Do NOT go back to `ui_mode: "embedded_page"` + `permissions.update_shipping_details`: Stripe refuses that parameter on newly activated accounts (the live account got a 400). It is per-account, not per-API-version — the `stripe` SDK pins `Stripe-Version` itself, so the sandbox that still accepts it was sent the same version. Pinning an older version is not a fix.
- The form collects the shipping address and stores it on the session itself. Rates come from Sendcloud, so the client quotes every completed address through the update route (src/app/api/checkout/update-shipping/route.ts) inside `actions.runServerUpdate`, and the route writes `shipping_options` ONLY.
- The form has no built-in way to reject an address, so our `confirm` handler is the gate: it calls `actions.confirm` only once the address currently in the form has a successful quote, else it shows "Spedizione non disponibile…" and does not confirm. Wallet buttons (`expressCheckout.paymentMethods`) are set to "never" because they bypass the server-side shipping update and could only pay the default IT rate.
